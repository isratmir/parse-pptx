...
def ShapeToSVG(shape):
    _dwg = svgwrite.Drawing(filename="svg/" + shape.name + ".svg",
                            size=(svg_coords(shape.width, shape.height)))
    _dwg.defs.add(_dwg.style('.non-scaling-stroke { vector-effect: non-scaling-stroke; }'))

    geometry = Geometry(shape)
    for path in geometry.path_list:
        # bbox = (shape.width, shape.height) if path.w is None else (path.w, path.h)
        # transform = Transform(shape, bbox)
        svg_path = _dwg.path(fill='#FF0000', stroke_width=1, class_='non-scaling-stroke')  # id='sss'
        # svg_path.matrix(*transform.svg_matrix())
        first_point = None
        current_point = None
        closed = False
